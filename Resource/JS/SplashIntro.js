window.SplashIntro = (function () {
    ...
    /** 检测 URL 是否已被浏览器缓存（同源静态资源）：
     *  命中缓存返回 true（直接本地加载），未命中返回 false（需要下载） */
    ...
    class SplashEngine {
        constructor(overlay) {
            ...
            this.downloadHint = overlay.querySelector('#downloadHint');
        ...
        /** 显示/隐藏下载阶段的提示文案（"第一次加载，需要一些时间"） */
        setDownloadHint(visible) {
            if (!this.downloadHint) return;
